...
data = pd.read_csv("test.csv", index_col="Date", parse_dates=True)
print(data.head())
...
plt.plot(test_data.index, actual_prices, color="blue", label=f"actual {company} price")
plt.plot(
    test_data.index[prediction_days:],
    predicted_prices,
    color="green",
...
real_data = real_data.reshape((real_data.shape[0], prediction_days, 1))
